...
enum Tree:
   ...
   *  @constructor create a new leaf with a value
   ...
   *
   *  @since 1.0 (2024-03-18)
   *  @constructor create a new node with a value and a list of children
   ...
   * @return `true` if the tree has the value, `false` otherwise
   *
   * @example {{{
   * Leaf(8).has(8)                           // true
   *
   ...
   *
   * @since 1.0 (2024-03-18)
   * @return a new tree with the values sorted in ascending order
   * @note The tree is sorted in a pre-order traversal.
   *
   * @example {{{
   * // 8
   * Leaf(8).sort
  ...
  def sort: Tree =
    ...
    def merge(tree: Tree, list: List[Int]): (Tree, List[Int]) = (tree, list) match
      case (Leaf(_), head :: tail) => (Leaf(head), tail)
      case (Node(_, children), head :: tail) =>
        val (newChildren, remaining) = children.foldLeft((List.empty[Tree], tail)) {
